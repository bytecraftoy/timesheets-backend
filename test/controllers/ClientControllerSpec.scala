...
class ClientControllerSpec
  ...
  "ClientController POST " should {
    ...
    "add the client and return the added client in subsequent get results" in {
      ...
      val request  = FakeRequest(POST, s"/clients?name=$clientName&email=some.email@server.invalid")
      ...
      val bodyTextContainsAddedClient = contentAsString(response2).contains(clientName)
  ...
  "ClientController POST " should {
    ...
    "fail if a client with the same e-mail is added more than one time" in {
      ...
      val time = Clock.systemUTC().instant()
      val clientName = s"Automated Test client $time"
      val request  = FakeRequest(POST, s"/clients?name=$clientName&email=duplicate.email@server.invalid")
        .withHeaders("Content-type" -> "application/json")
      val response = route(app, request).get
      status(response) mustBe OK
      contentType(response) mustBe Some("application/json")
      ...
      val request2  = FakeRequest(POST, s"/clients?name=$clientName2&email=duplicate.email@server.invalid")
        .withHeaders("Content-type" -> "application/json")
      val response2 = route(app, request2).get
  ...
  "ClientController GET by ID" should {
    ...
    "return JSON data " in {
      val request  = FakeRequest(GET, "/clientsbyid?id=1bb44a7e-cd7c-447d-a9e9-26495b52fa88")
      val response = route(app, request).get
